...
environment_dir="$path/../referee.environment"
. "$environment_dir"
...
testImagesDirectory="$LPCVC_Test_Data/IMG"
testGroundTruthImagesDirectory="$LPCVC_Test_Data/GT"
testImages=$(ls $testImagesDirectory)
summed_dice=0
count=600
start_time=$(date +%s)
output=$(timeout $TIMEOUT python3.6 -W ignore $solution -i ${testImagesDirectory} -o $eval 2>/dev/null )
time_output=$(echo "$output" | grep -oE '[0-9]+\.[0-9]+')
echo $output
exit_code=$?
end_time=$(date +%s)
echo $exit_code
